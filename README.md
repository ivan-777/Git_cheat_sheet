...
------
Пример: feature(MainActivity.class): add WebView
Closes #99
```
...
#### Интерактивная правка коммитов
``` bash
git rebase -i HEAD~3   # крайнии 3 коммита
git rebase --abort     # отбой, если передумали
```
...
``` bash
git checkout master       # переключиться те переместить HEAD на коммит, на который указывает master, рабочую директорию вернуть к состоянию этого коммита  
git pull                  # получаем обновления из удаленного master
git checkout new_branch   # переключиться на ветку разработки new_branch
# если ветка public
  git merge master        # создать коммит слияния в new_branch со всеми обновлениями из master 
# если ветка privat  
  git rebase master       # new_branch будет смещена вверх, как если бы мы только что создали её из последней версии master
```

#### Обновить локальную ветку v2 
``` bash
# checkout в ветку в которую нужно влить обновления
# если ветка public (коммит слияния)
  git pull origin master             # влить изменения с удалённого репозитория (только указанная ветка)
# если ветка privat
  git pull --rebase origin master
  # или
  git fetch                         # скачать к себе в локальный репозиторий обновления внешнего репозитория 
  git rebase master                 # пересадим ветку на актуальное состояние
```

#### Жестко все локально перезаписать 
``` bash 
git fetch --all                  # скачать все ветки с удаленного репозитория, но не сливать со своими ветками
git reset --hard origin/master   # отменить изменения  (неотслеживаемые файлы останутся на месте)
